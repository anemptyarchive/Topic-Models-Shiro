...
for(OI in 1:OutIter) { ## (イタレーション)
  ...
  for(d in 1:M) { ## (各文書)
    ...
    while(abs_err >= Abs_Err && II <= InIter) { ## (インナーイタレーション)
      ...
      tmp_q_z_kv <- exp(term1_kv) * exp(term2_k)
      # 正規化
      z_dv_k[d, , ] <- t(tmp_q_z_kv) / apply(tmp_q_z_kv, 2, sum)
...
for(OI in 1:(OutIter + 1)) {
  # データフレームに変換
  tmp_trace_xi_theta <- cbind(
    as.data.frame(trace_xi_theta[, , OI]), 
    doc = as.factor(1:M), # 文書番号
    Iter = OI - 1 # 試行回数
  )
  tmp_trace_xi_phi <- cbind(
    as.data.frame(trace_xi_phi[, , OI]), 
    topic = as.factor(1:K), # トピック番号
    Iter = OI - 1 # 試行回数
  )
  # 結合
